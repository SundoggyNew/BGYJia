//
//  BGYChangeIdCardCodeViewController.m
//  BGYJia
//
...
@interface BGYChangeIdCardCodeViewController ()
...
@implementation BGYChangeIdCardCodeViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    self.inputView.text = [User runUser].idcard;
}
...
-(void)setUpNameChangeSystemUI
{
    ...
    [[self.navgationRightButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        ...
        hud.labelText = @"正在更新身份证号...";
        
        if ([self.inputView getTheInputFieldText].length < 1) {
            
            DQAlertView * alert = [[DQAlertView alloc] initWithTitle:@"身份证号不能为空！" message:nil cancelButtonTitle:@"确定" otherButtonTitle:nil];
        ...
        BOOL haveRightIdCardCode = [RegularTools validateIdentityCard:[self.inputView getTheInputFieldText]];
        
        if (!haveRightIdCardCode) {
            
            DQAlertView * alert = [[DQAlertView alloc] initWithTitle:@"身份证号不正确！" message:nil cancelButtonTitle:@"确定" otherButtonTitle:nil];
        ...
        @weakify(self);
        [[JCNetWorking runNetWorking] changeUserInfoWithKey:@"idcard" value:[self.inputView getTheInputFieldText] success:^(NSDictionary *dictionary) {
            @strongify(self);
